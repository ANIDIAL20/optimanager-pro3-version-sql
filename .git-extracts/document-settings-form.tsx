...
// ÔöÇÔöÇ Demo data for live preview ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
const DEMO_SHOP = {
  nom: 'OptiqVision Pro',
  ...
  paymentMethods: ['Esp├¿ces', 'Ch├¿que', 'Carte'],
};
...
const DEMO_ITEMS_FACTURE = [
  ...
  { id: '3', description: 'Essentiel ├ëtui +Chiffon', quantite: 1, prixUnitaire: 80, total: 80, tvaRate: 20 },
];

function buildDemo(type: 'FACTURE' | 'DEVIS' | 'BON DE COMMANDE' | 'RE├çU', shop: typeof DEMO_SHOP): StandardDocumentData {
  if (type === 'RE├çU') {
    return {
      type: 'RE├çU',
      ...
      paiements: [
        { date: new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString(), mode: 'Esp├¿ces', montant: 500 },
  ...
  if (type === 'BON DE COMMANDE') {
    return {
      ...
      commandeDetails: {
        lieuLivraison: `${shop.nom} ÔÇö ${shop.adresse}`,
        dateLivraisonSouhaitee: new Date(Date.now() + 10 * 24 * 60 * 60 * 1000).toISOString(),
        observations: 'Patient: Karim El Hassani ÔÇö Dr. Mohamed Alaoui ÔÇö Ord. du 16/02/2026\nLivraison en une seule fois svp. Traitement AR obligatoire.',
      },
      items: [
        {
          id: '1',
          description: 'Verre Essilor Progressif Varilux ÔÇö OD',
          marque: 'Essilor',
        ...
        {
          id: '2',
          description: 'Verre Essilor Progressif Varilux ÔÇö OG',
          marque: 'Essilor',
          modele: 'Varilux X Series 1.6',
          quantite: 1,
          prixUnitaire: 375,
...
const TEMPLATES: { id: TemplateId; label: string; defaultColor: string }[] = [
  ...
  { id: 'elegant',  label: '├ël├®gant',      defaultColor: '#92400e' },
];
...
export function DocumentSettingsForm({ shopId, initialShopProfile, initialConfig }: DocumentSettingsFormProps) {
  ...
  const [previewType, setPreviewType] = useState<'FACTURE' | 'DEVIS' | 'BON DE COMMANDE' | 'RE├çU'>('FACTURE');
  const [isPending, startTransition] = useTransition();

  // ÔöÇÔöÇ Real Lens Order (Bon Cmd) preview data ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  ...
  // ÔöÇÔöÇ Dynamic A4 scale ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  // 794px Ôëê 210mm at 96 dpi (A4 width). We keep a small padding factor so the
  ...
  // ÔöÇÔöÇ Sync from DB on mount ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  ...
  const handleSave = () => {
    startTransition(async () => {
      try {
        ...
        toast.success('Ô£à Configuration enregistr├®e avec succ├¿s');
      } catch (err: any) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[42%_58%] gap-6 h-[calc(100vh-210px)] min-h-[600px]">
      ...
      {/* ÔöÇÔöÇ LEFT: Controls Panel ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
      <Card className="flex flex-col h-full overflow-hidden border border-slate-200 shadow-sm">
        ...
        <CardContent className="flex-1 overflow-y-auto p-5 space-y-6">
          ...
          {/* ÔöÇÔöÇ SECTION B: Colors & Typography */}
          <section>
            <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-500 border-b pb-1.5 mb-3">
              B ÔÇö Couleurs &amp; Typographie
            </h3>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <p className="text-[10px] text-slate-400">Textes secondaires, d├®tails</p>
              </div>
            </div>
          ...
          {/* ÔöÇÔöÇ SECTION C: Header Layout */}
          <section>
            <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-500 border-b pb-1.5 mb-3">
              C ÔÇö Mise en page du Header
            </h3>
            <Label className="text-xs font-medium mb-2 block">Position du logo</Label>
            <div className="flex gap-2">
              {([ 
                { id: 'logo-left',   icon: AlignLeft,   label: 'ÔùÇ Logo   Titre' },
                { id: 'logo-center', icon: AlignCenter, label: 'Ôû╝ Logo centr├®' },
                { id: 'logo-right',  icon: AlignRight,  label: 'Titre   Logo ÔûÂ' },
          ...
          {/* ÔöÇÔöÇ SECTION D: Visibility */}
          <section>
            <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-500 border-b pb-1.5 mb-3">
              D ÔÇö Visibilit├® des informations
            </h3>
            <div className="grid grid-cols-2 gap-x-4 gap-y-2.5">
              {([
                ...
                { key: 'showStamp',        label: 'Cachet (├á venir)' },
          ...
          <section>
            <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-500 border-b pb-1.5 mb-3">
              E ÔÇö Texte Pied de page (optionnel)
            </h3>
            <Textarea
              value={config.footerText ?? ''}
              onChange={e => update({ footerText: e.target.value })}
              placeholder="Ex: SARL au capital de 10.000 DH ÔÇö RC 12345 ÔÇö Casablanca"
              rows={2}
              className="text-xs"
            />
            <p className="text-[10px] text-slate-400 mt-1">Remplace les mentions l├®gales dans le footer</p>
          </section>
      ...
      {/* ÔöÇÔöÇ RIGHT: Live Preview ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
      <div className="flex flex-col h-full bg-slate-900 rounded-xl overflow-hidden shadow-2xl border border-slate-700">
        ...
        <div className="bg-slate-800 px-4 py-2.5 flex justify-between items-center border-b border-slate-700 shrink-0">
          <div className="flex items-center gap-2 text-white text-sm font-medium">
            ...
            Aper├ºu en direct
            <span className="text-[10px] text-slate-400 font-normal ml-1">ÔÇö mise ├á jour instantan├®e</span>
            ...
            {previewType === 'BON DE COMMANDE' && (
              lensPreviewLoading
                ...
                : lensPreviewFetched && !lensPreviewData
                  ? <span className="text-[9px] bg-amber-900/60 text-amber-300 px-1.5 py-0.5 rounded ml-1">Aper├ºu d├®mo ÔÇö aucune commande</span>
                  : lensPreviewData
                    ? <span className="text-[9px] bg-green-900/60 text-green-300 px-1.5 py-0.5 rounded ml-1">Donn├®es r├®elles Ô£ô</span>
                    : null
            )}
          </div>
          {/* Doc type toggle */}
          <div className="flex bg-slate-700 rounded-lg p-0.5">
            {(['FACTURE', 'DEVIS', 'BON DE COMMANDE', 'RE├çU'] as const).map(t => (
              <button
                key={t}
              ...
              >
                {t === 'BON DE COMMANDE' ? 'Bon Cmd' : t === 'FACTURE' ? 'Facture' : t === 'DEVIS' ? 'Devis' : 'Re├ºu'}
              </button>
            ))}
          </div>
        </div>
