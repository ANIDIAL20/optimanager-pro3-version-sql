...
import { useFirestore, useDoc, useFirebase, updateDocumentNonBlocking } from '@/firebase';
...
export default function EditClientPage() {
    ...
    const clientRef = React.useMemo(
        () => (firestore && user && id ? doc(firestore, `stores/${user.uid}/clients`, id) : null),
        [firestore, user, id]
    );

    const { data: client, isLoading, error } = useDoc<Client>(clientRef);
    ...
    React.useEffect(() => {
        if (client) {
            form.reset({
                ...
                sexe: (client.sexe === 'M' ? 'Homme' : client.sexe === 'F' ? 'Femme' : client.sexe) || 'Homme',
                cni: client.cni || '',
                email: client.email || '',
                telephone1: client.telephone1 || client.phone || '',
                telephone2: client.telephone2 || '',
                ville: client.ville || '',
                adresse: client.adresse || '',
                mutuelle: client.mutuelle || '',
                dateNaissance: client.dateNaissance ? new Date(client.dateNaissance) : undefined,
            });
        }
    }, [client, form]);
    ...
    const onSubmit = async (data: ClientFormValues) => {
        ...
        try {
            ...
            await updateDocumentNonBlocking(clientRef, updatedData);
