  ...
  const onSubmit = async (data: ClientFormValues) => {
    ...
    startTransition(async () => {
      try {
        ...
        await addDocumentNonBlocking(clientsRef, clientData);

        toast({
          title: 'Client Ajouté',
          description: `Le client ${data.prenom} ${data.nom} a été créé avec succès.`,
        });
          ...
          <Card>
            ...
            <CardFooter>
              <Button type="submit" disabled={form.formState.isSubmitting || isPending}>
                {(form.formState.isSubmitting || isPending) && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                Ajouter Client
              </Button>
            </CardFooter>
          </Card>
